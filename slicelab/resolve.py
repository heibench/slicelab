...
def _promote_readback(readback: Redacted, destination: Path) -> None:
    """The redacted copy, written where the author asked for it.

    Promoted on every adjudicated outcome, not only on `sliced` -- which is where this
    parts from D7's letter, and D31 says why: for G-code a partial artifact is
    ...
    A failure to write it establishes nothing about the intent, so it is an
    environment fault -- an uncaught traceback here exited 1, which is `refused`, and
    put `Traceback` where D14 requires the outcome word.
    """
    try:
        promote(readback.text.encode("utf-8"), destination)
    except PromotionError as unwritable:
        raise ResolveError(str(unwritable)) from unwritable
